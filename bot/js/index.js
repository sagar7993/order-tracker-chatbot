// First route to show
var GLOBALSTATE = {
    route: '.list-text'
};

var date = new Date(); var hours = String(date.getHours()); var minutes = String(date.getMinutes());
if(hours.length == 1) { hours = "0" + hours; } if(minutes.length == 1) { minutes = "0" + minutes; }
var time = hours + " : " + minutes; $(".time").html("<b>" + time + "</b>");

// Set first Route
setRoute(GLOBALSTATE.route);
$('.nav > li[data-route="' + GLOBALSTATE.route + '"]').addClass('active');

// Have to Delegate ripple due to dom manipulation (add)
$('ul.mat-ripple').on('click', 'li', function(event) {
    if ($(this).parent().hasClass('tiny')) {
        var $ripple = $('<div class="ripple tiny"></div>');
    } else {
        var $ripple = $('<div class="ripple"></div>');
    }
    var x = event.offsetX;
    var y = event.offsetY;
    var $me = $(this);
    $ripple.css({
        top: y,
        left: x
    });
    $(this).append($ripple);
    setTimeout(function() {
        $me.find('.ripple').remove();
    }, 530)
});

// Set Name
setName(localStorage.getItem('username'));

// Dyncolor ftw
if (localStorage.getItem('color') !== null) {
    var colorarray = JSON.parse(localStorage.getItem('color'));
    stylechange(colorarray);
} else {
    var colorarray = [15, 157, 88]; // 15 157 88 = #0f9d58
    localStorage.setItem('color', JSON.stringify(colorarray));
    stylechange(colorarray);
}

// Helpers
function setName(name) {
    $.trim(name) === '' || $.trim(name) === null ? name = 'John Doe' : name = name;
    $('h1').text(name);
    localStorage.setItem('username', name);
    $('#username').val(name).addClass('used');
    $('.card.menu > .header > h3').text(name);
}

// Stylechanger
function stylechange(arr) {
    var x = 'rgba(' + arr[0] + ',' + arr[1] + ',' + arr[2] + ',1)';
    $('#dynamic-styles').text('.dialog h3 {color: ' + x + '} .i-group input:focus ~ label,.i-group input.used ~ label {color: ' + x + ';} .bar:before,.bar:after {background:' + x + '} .i-group label {color: ' + x + ';} ul.nav > li.active {color:' + x + '} .style-tx {color: ' + x + ';}.style-bg {background:' + x + ';color: white;}@keyframes navgrow {100% {width: 100%;background-color: ' + x + ';}} ul.list li.context {background-color: ' + x + '}');
}

function closeModal() {
    $('#new-user').val('');
    $('.overlay').removeClass('add');
    $('.floater').removeClass('active');
    $('#contact-modal').fadeOut();
    $('#contact-modal').off('click', '.btn.save');
}

function setModal(mode, $ctx) {
    var $mod = $('#contact-modal');
    switch (mode) {
        case 'add':
            $mod.find('h3').text('Add Contact');
            break;
        case 'edit':
            $mod.find('h3').text('Edit Contact');
            $mod.find('#new-user').val($ctx.text()).addClass('used');
            break;
    }
    $mod.fadeIn();
    $('.overlay').addClass('add');
    $mod.find('#new-user').focus();
}

$('.mdi-arrow-left').on('click', function() {
    $('.shown').removeClass('shown');
    setRoute('.list-text');
});

// Set Routes - set floater
function setRoute(route) {
    GLOBALSTATE.route = route;
    $(route).addClass('shown');
    if (route !== '.list-account') {
        $('#add-contact-floater').addClass('hidden');
    } else {
        $('#add-contact-floater').removeClass('hidden');
    }
    if (route !== '.list-text') {
        $('#chat-floater').addClass('hidden');
    } else {
        $('#chat-floater').removeClass('hidden');
    }
    if (route === '.list-chat') {
        $('.mdi-menu').hide();
        $('.mdi-arrow-left').show();
        $('#content').addClass('chat');
    } else {
        $('#content').removeClass('chat');
        $('.mdi-menu').show();
        $('.mdi-arrow-left').hide();
    }
}

// Colorpicker
var cv = document.getElementById('colorpick');
var ctx = cv.getContext('2d');
var img = new Image();
// Meh .. Thx 2 Browser Security i need BASE64
img.src = 'data:image/gif;base64,';
img.onload = function() {
    ctx.drawImage(img, 0, 0, img.width, img.height);
};

//todo optimize
$('#username').on('blur', function() {
    setName($(this).val());
    $('.card.menu > .header > img').addClass('excite');
    setTimeout(function() {
        $('.card.menu > .header > img').removeClass('excite');
    }, 800);
});

// Dirty Colorpicker
$('#colorpick').on('mousedown', function(eventDown) {
    var x = eventDown.offsetX;
    var y = eventDown.offsetY;
    if (eventDown.button === 0) {
        $('.card.menu > .header > img').addClass('excite');
        setTimeout(function() {
            $('.card.menu > .header > img').removeClass('excite');
        }, 800);
        var imgData = ctx.getImageData(x, y, 1, 1).data;
        localStorage.setItem('color', JSON.stringify(imgData));
        stylechange(imgData);
    }
});

var step = 0; var action = 0;

//SEND MESSAGE BY PRESSING SEND
$('.mdi-send').on('click', function() {
    showMessage($('.chat-input').val(), 'right', true);
});

//SHOW USER MESSAGE FUNCTION
function showMessage(message, side, sendToServer) {
    if(message.trim() != "") {
        var $currentmessage = $('ul.chat > li > #current'); $currentmessage.removeAttr("id");
        if(side == 'right') { var style = ' style="flex-direction:row-reverse;"'; } else { var style = ""; }
        var $chatmessage = 
        '<li' + style +  '>' + 
            '<img src="images/profile.png">' + 
            '<div class="message" id="current">' + message + '</div>' + 
        '</li>';
        $('ul.chat').append($chatmessage); $('.chat-input').val('');
        if(sendToServer && step == -1) { step = 0; action = 0; }
        else if(sendToServer && step == 0) { analyzeMessage(message); }
        else if(sendToServer && step == 1) { sendMobileNumber(message); }
        // try { $('html, body').animate({ scrollTop: $("#chat-input").offset().top }, 1000); } catch(err) { console.log(err); }
        location.href = "#current";
    } else {
        $('.chat-input').val('');
    }
}

//ANALYZE MESSAGE
function sendMobileNumber(mobileNumber) {
    $.ajax({
        method: "POST",
        url: "http://10.130.202.153:4730/sendMobileNumber",
        data: { mobileNumber: mobileNumber, step: step, action: action }
    }).done(function(response) {
        var message = response.message; step = response.step; action = response.action;
        if(action == 1 || action == 2) {
            showMessage("Apologies for the inconvenience.", 'left', false);
            showMessage("Fetching your most recent order details...", 'left', false);
        }
        showMessage(message, 'left', false);
        if(step == 2) { showActionButton(response); }
    }).fail(function(error) {
        console.log(error); step = 0; action = 0; showMessage("Sorry... Something went wrong", 'left', false);
    });
}

//ANALYZE MESSAGE
function analyzeMessage(message) {
    $.ajax({
        method: "POST",
        url: "http://10.130.202.153:4730/analyze",
        data: { text: message.toLowerCase(), step: step, action: action }
    }).done(function(response) {
        var message = response.message; step = response.step; action = response.action;
        if(response.step == 1) {
            showMessage(message, 'left', false);
        } else {
            if(response.action == 1 || response.action == 2) {
                showMessage("Apologies for the inconvenience.", 'left', false);
                showMessage("Fetching your most recent order details...", 'left', false);
            }
            showMessage(message, 'left', true);
            if(step == 2) { showActionButton(response); }
        }
    }).fail(function(error) {
        console.log(error); step = 0; action = 0; showMessage("Sorry... Something went wrong", 'left', false);
    });
}

//SHOW ACTION BUTTON FUNCTION
function showActionButton(response) {
    if(response.action == 1 || response.action == 2 || response.action == 3) {
        var $html = 
        '<li align="center" style="display:inline-block;" class="buttons">' + 
            '<button class="btn style-bg cancel" onclick="takeAction(' + response.action + ', ' + response.step + ')">' + response.buttonText + '</button>' + 
            '<button class="btn style-bg ignore" onclick="ignoreAction()" style="margin-left:10px;">Ignore</button>' + 
        '</li>';
        $('ul.chat').append($html); $('.meta-bar').hide('slow'); $('.chat-input').val('');
    }
}

//TAKE ACTION FUNCTION
function takeAction(action, step) {
    if(action == 1 || action == 2) {
        $.ajax({
            method: "POST",
            url: "http://10.130.202.153:4730/takeAction",
            data: { action: action, step: step }
        }).done(function(response) {
            var message = response.message; step = response.step; action = response.action;
            showMessage(message, 'left', true);
            $('.meta-bar').show('slow'); $('.chat-input').val(''); $('.buttons').hide('slow');
        }).fail(function(error) {
            console.log(error); step = 0; action = 0; showMessage("Sorry... Something went wrong", 'left', false);
        });
    } else {
        step = 0; action = 0; $('.meta-bar').show('slow'); $('.chat-input').val(''); $('.buttons').hide('slow');
    }
}

//IGNORE ACTION FUNCTION
function ignoreAction() {
    $('.meta-bar').show('slow'); $('.chat-input').val(''); $('.buttons').hide('slow'); step = 0; action = 0;
}

//SEND MESSAGE BY PRESSING ENTER
$('.chat-input').on('keyup', function(event) {
    event.preventDefault();
    if (event.which === 13) {
        $('.mdi-send').trigger('click');
    }
});

//SELECT A CHAT WINDOW
$('.list-text > ul > li').on('click', function() {
    $('ul.chat > li').eq(1).html(
        '<img src="' + $(this).find('img').prop('src') + '">' + 
        '<div class="message"><p>'
    );
    // timeout just for eyecandy...
    setTimeout(function() {
        $('.shown').removeClass('shown');
        $('.list-chat').addClass('shown');
        setRoute('.list-chat');
        $('.chat-input').focus();
    }, 300);
});

$('#head').on('click', '.mdi-fullscreen', function() {
    $(this).removeClass('mdi-fullscreen').addClass('mdi-fullscreen-exit');
    $('#hangout').css({
        width: '900px'
    });
});

$('#head').on('click', '.mdi-fullscreen-exit', function() {
    $(this).removeClass('mdi-fullscreen-exit').addClass('mdi-fullscreen');
    $('#hangout').css({
        width: '400px'
    });
});

// menuclick
$('#head .mdi-menu').on('click', function() {
    $('.menu').toggleClass('open');
    $('.overlay').toggleClass('add');
});

// viewtoggle > 1000
$('#head .mdi-chevron-down').on('click', function() {
    if ($('#hangout').hasClass('collapsed')) {
        $(this).removeClass('mdi-chevron-up').addClass('mdi-chevron-down');
        $('#hangout').removeClass('collapsed');
    } else {
        $(this).removeClass('mdi-chevron-down').addClass('mdi-chevron-up');
        $('#hangout').addClass('collapsed');
    }
});

// Filter
$('.search-filter').on('keyup', function() {
    var filter = $(this).val();
    $(GLOBALSTATE.route + ' .list > li').filter(function() {
        var regex = new RegExp(filter, 'ig');
        if (regex.test($(this).text())) {
            $(this).show();
        } else {
            $(this).hide();
        }
    });
});

// killit
$('#contact-modal').on('click', '.btn.cancel', function() {
    closeModal();
});